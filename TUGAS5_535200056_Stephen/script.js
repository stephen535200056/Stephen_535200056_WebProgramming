const API_KEY = '1a699bc0e899bcf15383f03665ab41f7';
...
$('#searchButton').click((e) => {
  ...
  $.get('https://api.themoviedb.org/3/search/movie', data)
    ...
    .always(() => {
      ...
      $('#searchButton').empty().removeAttr('disabled').append('Search');
    });
});
...
function createCell(movie) {
  ...
  td.append(overview);
  // rating
  const rating = $('<span class="badge badge-success p2"></span>').text(
    `Rating: ${movie.vote_average}`
  );
  td.append(rating);
  // genres
  movie.genre_ids.forEach((id) => {
    const genre = $('<span class="badge badge-warning ml-2 p2"></span>').text(
      genres[id]
    );
    td.append(genre);
  });
